...
export default function AllGuns() {
  ...
  const [subCat, setSubCat] = useState('All'); // New state for sub-category
  ...
  useEffect(() => {
    ...
    const subCatFromQuery = queryParams.get('subCat') || 'All';
    setCategory(categoryFromQuery);
    setSubCat(subCatFromQuery);
    filterGuns(searchTerm, categoryFromQuery, subCatFromQuery);
  }, [location.search, searchTerm]); // Add dependencies to re-filter on search or category/subCat change

  const categories = ['All', ...new Set(gunsData.map(gun => gun.category))];
  const subCats = ['All', ...new Set(gunsData.map(gun => gun.subCat))]; // Extract unique subCats
  ...
  const handleSearch = (event) => {
    ...
    setSearchTerm(term);
    filterGuns(term, category, subCat);
  };

  const handleCategoryClick = (selectedCategory) => {
    setCategory(selectedCategory);
    filterGuns(searchTerm, selectedCategory, subCat);
    navigate(`/all-pets?category=${encodeURIComponent(selectedCategory)}&subCat=${encodeURIComponent(subCat)}`); // Update URL
  };

  const handleSubCatClick = (selectedSubCat) => { // New sub-category click handler
    setSubCat(selectedSubCat);
    filterGuns(searchTerm, category, selectedSubCat);
    navigate(`/all-pets?category=${encodeURIComponent(category)}&subCat=${encodeURIComponent(selectedSubCat)}`); // Update URL
  };

  const filterGuns = (term, selectedCategory, selectedSubCat) => {
    const filtered = gunsData.filter(gun => {
      ...
      const matchesSubCat = selectedSubCat === 'All' || gun.subCat === selectedSubCat; // SubCat filter logic

      return matchesSearchTerm && matchesCategory && matchesSubCat;
    });
    ...
    <AllGunsStyles>
      ...
      <div className="sub-categories">
        {subCats.map((sub, index) => (
          <p
            key={index}
            className={`filter ${subCat === sub ? 'active' : ''}`}
            onClick={() => handleSubCatClick(sub)} // Use new sub-category click handler
          >
            {sub}
          </p>
        ))}
          ...
          <div className="card-wrapper" key={index} onClick={() => handleCardClick(gun.name)}>
            ...
            <div className='info'>
              ...
              <div className='info-right'>
                ...
                <span className='text' style={{textTransform: 'uppercase'}}>Vaccinated</span> {gun.subCat}<br />
              </div>
            </div>
...
const AllGunsStyles = styled.div`
  ...
  @media (max-width: 768px) {
    /* Add any necessary adjustments for smaller screens */
  }
`;
